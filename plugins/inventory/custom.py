        ...
        inventory_data = {
            ...
            '_meta': {
                'hostvars': {
                    ...
                    'rhel9-vm': {
                        'ansible_host': 'rhel9-vm.rh-lab.labs',
                    }
                }
            }
        }
            ...
            if group_by_ip:
                if hostvars['ansible_host'] != "rhel9-vm.rh-lab.labs":
                    if ipaddress.ip_address(hostvars['ansible_host']) in ipaddress.ip_network('192.168.1.0/24'):
                        ...
                        inventory.add_child("group_a", host)  # Add host to the specified group
                    if ipaddress.ip_address(hostvars['ansible_host']) in ipaddress.ip_network('192.168.2.0/24'):
                        inventory.add_group("group_b")
                        inventory.add_child("group_b", host)  # Add host to the specified group
                else:
                    inventory.add_group("group_c")
                    inventory.add_child("group_c", host)  # Add host to the specified group
